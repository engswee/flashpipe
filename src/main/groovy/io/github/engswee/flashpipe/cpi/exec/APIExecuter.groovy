...
abstract class APIExecuter {
    ...
    APIExecuter() {
        ...
        logger.debug("HOST_OAUTH = ${this.oauthTokenHost}")
        logger.debug("HOST_TMN = ${tenantMgmtHost}")
        if (this.oauthTokenHost) {
            ...
            logger.debug("OAUTH_CLIENTID = ${oauthClientID}")
//            logger.debug("OAUTH_CLIENTSECRET = ${oauthClientSecret}")
            logger.debug("HOST_OAUTH_PATH = ${oauthTokenPath}")
            ...
            logger.debug("BASIC_USERID = ${basicAuthUser}")
//            logger.debug("BASIC_PASSWORD = ${basicAuthPassword}")
