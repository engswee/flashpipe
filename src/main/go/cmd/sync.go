...
tenant to a Git repository.`,
	...
	syncCmd.Flags().String("dir-gitsrc", "", "Base directory containing contents of Integration Flow(s) [or set environment GIT_SRC_DIR]")
	syncCmd.Flags().String("dir-work", "/tmp", "Working directory for in-transit files [or set environment WORK_DIR]")
	syncCmd.Flags().String("dirnamingtype", "ID", "Name IFlow directories by ID or Name. Allowed values: ID, NAME [or set environment DIR_NAMING_TYPE]")
	syncCmd.Flags().String("drafthandling", "SKIP", "Handling when IFlow is in draft version. Allowed values: SKIP, ADD, ERROR [or set environment DRAFT_HANDLING]")
	syncCmd.Flags().String("ids-include", "", "List of included IFlow IDs [or set environment INCLUDE_IDS]")
	syncCmd.Flags().String("ids-exclude", "", "List of excluded IFlow IDs [or set environment EXCLUDE_IDS]")
	...
	syncCmd.Flags().String("normalise-manifest-action", "NONE", "Action for normalising IFlow ID & Name in MANIFEST.MF. Allowed values: NONE, ADD_PREFIX, ADD_SUFFIX, DELETE_PREFIX, DELETE_SUFFIX [or set environment NORMALISE_MANIFEST_ACTION]")
	syncCmd.Flags().String("normalise-manifest-prefixsuffix", "", "Prefix/suffix used for normalising IFlow ID & Name in MANIFEST.MF [or set environment NORMALISE_MANIFEST_PREFIX_SUFFIX]")
